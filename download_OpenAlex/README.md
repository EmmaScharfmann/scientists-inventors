...
* The notebook "Download_OpenAlex.ipynb" describes how to download, flatten OpenAlex data and how to create, load and index Postgres tables.
    ...
    * the section "OpenAlex authors" provides similar code to flatten the author's data, and create, load and index the author table into a postgres database. 
...
* The python file "flatten_data_works.py" flattens the works from OpenAlex and save the flat files.

* Note that downloading a full OpenAlex snapshot requires considerable memory (~300GB before loading the data to the database). 
