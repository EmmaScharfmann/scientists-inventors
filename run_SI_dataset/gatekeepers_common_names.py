...
def get_pd_table_comparison(last_name, first_name , dic_inventors, dic_inventor_first_names):
...
def get_dic_test(last_name, first_name , dic_inventors, dic_inventor_first_names):
    
    
    #get the author ids, inventor ids, relative papers and patents, and similarity scores between the author and inventor names 
    dic_comparison , dic_ids , dic_authors = get_pd_table_comparison(last_name, first_name , dic_inventors, dic_inventor_first_names)[:3]
...
def prediction(func_input, dic_inventors, dic_inventor_first_names):

    name , first_name  , filelocation , filewrite = func_input
    
    dic_res = get_dic_test(name, first_name , dic_inventors, dic_inventor_first_names)
    ...
    file_object.write(name + "%" + first_name +  "\n")
    file_object.close()
...
## load the names that are already done (stored in the folder "run_SI_dataset/Names/name_matched_*"
...
## load the first names that are already done (stored in the folder "run_SI_dataset/Names/first_name_matched_*"
files_first_names = glob.glob(main_path + "run_SI_dataset/Names/first_name_matched_*")

first_names_done = set()
for file in files_names:
    file_text = open(file , "r")
    for line in file_text:
        first_names_done.add(line.replace("\n" , "").lower())
...
def multi_prediction(i):
    ...
    file_monitor = main_path + "run_SI_dataset/Monitor/name_matched_errors_" + str(i) + ".txt"
    file_object = open(file_monitor, 'a')
    file_object.write("start" + "\n")
    file_object.close()
    
    errors = {'junior', 'senior', 'sr'}
    list_names = list(set(list(dic_last_names.keys())) - names - errors) 
    list_names.sort()
    
    filename = main_path + "run_SI_dataset/test/first_name_matched_" + str(i) +  ".tsv"
    filewrite =  main_path + "run_SI_dataset/test/first_name_matched_" + str(i) +  ".txt"

    ## identify first names not done
    for name in list_names[:1]:

        print(name)
        
        names_done = set( [ elem.split("%")[1] for elem in first_names_done if elem.split("%")[0] == name])

        ## get inventors corresponding to the last name 
        dic_inventors, dic_inventor_first_names = get_PatentsView_inventors_ids(name)
        first_names = list(dic_inventor_first_names.keys())[:12]
        first_names = list(set(first_names) - names_done)
        first_names.sort()

        ## distribute the first names 
        func_inputs = [ ( name , first_names[k] , filename , filewrite ) for k in range(i , len(first_names) , workers) ]
        ...
        ## for each first name, apply the process to the first name - last name 
        for func_input in func_inputs:
               
            try:
                res = prediction( func_input , dic_inventors, dic_inventor_first_names)
                
            except:
                
                file_error = main_path + "run_SI_dataset/Errors/first_name_matched_errors_" + str(i)  + ".txt"
                file_object = open(file_error, 'a')
                file_object.write(func_input[1] + "%" + last_name + "\n")
                file_object.close()
