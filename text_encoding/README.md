...
<<<<<<< HEAD
This section requires the full data (OpenAlex + PatentsView) to be loaded in a Postgres database (username and password are required to run the codes, please see the folder download_OpenAlex and download_PatentsView). 
=======
...
<<<<<<< HEAD
    * The section "Create table to store patents' embeddings" provides the code to create a Postgres SQL table to store the titles and abstracts encoding.
=======
