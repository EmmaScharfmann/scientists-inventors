...
* load data
use /home/fs01/spec1142/Emma/GateKeepers/regressions/patents_cites_novelty_1yearwindow_05.dta, replace
...
reghdfe age_citation has_active_SI has_career_SI tot_inventors  , absorb(i.year#i.cpc_class_int#i.assignee_id) 
estimates store age_citation
summarize age_citation has_active_SI has_career_SI tot_inventors if e(sample)
...
use /home/fs01/spec1142/Emma/GateKeepers/regressions/science_papers_cites_novelty_1yearwindow_05.dta, replace
...
reghdfe mean_age_backward_cite has_active_SI has_career_SI tot_authors , absorb(i.pubyear#concept1_int#i.institution_int) 
estimates store mean_age_backward_cite
summarize mean_age_backward_cite has_active_SI has_career_SI tot_authors if e(sample)
...
esttab cites_papers novelty_papers  cites_patents novelty_patents using /home/fs01/spec1142/Emma/GateKeepers/regressions/regression_level1_conf05_4models_1yearwindow_indicator.tex, mtitles("Papers citations" "Papers novelty"  "Patents citations" "Patents novelty" ) keep(has_active_SI has_career_SI tot_authors ) star(* 0.10 ** 0.05 *** 0.01) collabels(none) label stats(r2 N , fmt(%9.4f %9.0f %9.0fc) labels("R-squared" "Number of observations" )) plain b(%9.4f) se(%9.4f) noabbrev se nonumbers lines parentheses replace fragment

esttab  age_citation  mean_age_backward_cite using /home/fs01/spec1142/Emma/GateKeepers/regressions/regression_other_effects_1yearwindow.tex, mtitles("Papers age cites" "Patents age cites" ) keep(has_active_SI has_career_SI tot_authors ) star(* 0.10 ** 0.05 *** 0.01) collabels(none) label stats(r2 N , fmt(%9.4f %9.0f %9.0fc) labels("R-squared" "Number of observations" )) plain b(%9.4f) se(%9.4f) noabbrev se nonumbers lines parentheses replace fragment
