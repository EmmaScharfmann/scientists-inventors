...
* The do files "regressions_1yearwindow_05.do", "regressions_3yearswindow_05.do", "regressions_5yearswindow_05.do" , and "regressions_02.do" provide the code to run the document level regression in stata for the high confidence SI dataset (corresponding to tables S4 to S7 of the SM). Similar regressions can be run for a different confidence threshold using this do file by changing the input file. 
* The do file "regressions_by_year_05.do" provides the code to run the career level regressions in stata for the high confidence SI dataset, which corresponds to figure S8 of the SM. Similar regressions can be run for a different confidence threshold using this do file by changing the input file.

* The notebook "regression_file.ipynb" also provides the code to generate figure S26 from the SM and figure 1 from the main paper.

* Note that all the data to run the regressions and reproduce the figures can be found in data_regressions. 
